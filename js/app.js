...
(function () {
      ...
      entries.forEach((entry) => {
        ...
        if (entry.target.hasAttribute('data-stagger')) {
          Array.from(entry.target.children).forEach((child) => {
            child.classList.toggle('visible', entry.isIntersecting);
          });
        }
        if (heroReady && entry.target.classList.contains('hero-copy') && entry.isIntersecting) {
          entry.target.querySelectorAll('.split-text .char').forEach((char) => {
  ...
  /* ────────────────────────────────────────────────
     ...
     Fills from 0→100% width as the user scrolls
     through the document height.
  ──────────────────────────────────────────────── */
